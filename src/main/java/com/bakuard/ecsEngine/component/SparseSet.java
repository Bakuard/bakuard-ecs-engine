...
public final class SparseSet implements CompPool {
	...
	@Override
	public <T> void merge(CompPool src, MergeCompPoolStrategy<T> strategy) {
		++actualModCount;

		for(int i = size - 1; i >= 0; --i) {
			Entity entity = entities[i];
			T currentComp = (T) comps[i];
			T srcComp = src.getComp(entity);
			T result = strategy.merge(entity, currentComp, srcComp);
			if(result == null) detachCompIgnoringPresence(entity, i);
			else comps[i] = result;
		}

		final SparseSet dest = this;
		src.<T>forEach((entity, comp) -> {
			if(!dest.hasComp(entity)) {
				T result = strategy.merge(entity, null, comp);
				if(result != null) dest.attachComp(entity, result);
			}
		});
	}
